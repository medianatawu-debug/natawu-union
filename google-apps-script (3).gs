...
const CONFIG={
  ...
  APPLICATION_TO:'kzn@natawu.net.za',
  APPLICATION_CC:'wc@natawu.net.za',
  APPLICATION_BCC:'sgift8083@gmail.com',
  ENQUIRY_TO:'kzn@natawu.net.za',
  ENQUIRY_CC:'wc@natawu.net.za',
  ENQUIRY_BCC:'sgift8083@gmail.com'
};
...
/* Neutralises spreadsheet formula injection: if a submitted value starts
   with =, +, -, or @, Sheets would otherwise try to evaluate it as a
   formula the moment someone opens that row. Prefixing with an
   apostrophe forces it to display as plain text instead, exactly like
   typing '= into a cell manually. Every value written to a sheet in
   this file goes through this first. */
...
function handleMembership(data){
  ...
  sh.appendRow(sanitizeRow(MEMBERSHIP_FIELDS.map(k=>data[k]??'')));
...
function handleContact(data){
  ...
  sh.appendRow(sanitizeRow(ENQUIRY_FIELDS.map(k=>k==='reference'?ref:k==='submittedAt'?now.toISOString():(data[k]||''))));
...
/* New sheet tabs default to 26 columns. The Applications tab needs more
   (43 fields), so grow the sheet before writing/formatting anything past
   column 26 — without this, Sheets throws "columns are out of bounds". */
function ensureColumns(sh,needed){
  const have=sh.getMaxColumns();
  if(have<needed) sh.insertColumnsAfter(have,needed-have);
}
...
function sendMembershipEmail(data,ref,now){
  ...
  MailApp.sendEmail({
    to:CONFIG.APPLICATION_TO,
    cc:CONFIG.APPLICATION_CC,
    bcc:CONFIG.APPLICATION_BCC,
    subject:subject,
...
function sendEnquiryEmail(data,ref,now){
  ...
  MailApp.sendEmail({
    ...
    cc:CONFIG.ENQUIRY_CC,
    bcc:CONFIG.ENQUIRY_BCC,
    subject:subject,
    body:plainBody,
